...
import { Trash2, Minus, Plus, Library, LayoutGrid, List, SlidersHorizontal, X, ArrowUpDown } from "lucide-react";
...
export default function Collection() {
  ...
  const cmcBuckets = useMemo(() => {
    ...
    return ["0","1","2","3","4","5","6","7+","?"]
      .filter((k) => b[k])
      .map((k) => ({ cmc: k, count: b[k] }));
  }, [cards]);

  const colorData = useMemo(() => {
    const totals: Record<string, number> = {};
    cards.forEach((c) => (c.colors ?? []).forEach((col) => { totals[col] = (totals[col] ?? 0) + c.quantity; }));
    return Object.entries(totals).map(([name, value]) => ({ name, value, fill: MANA_HEX[name] ?? "#888" }));
  }, [cards]);
              ...
              <div className="flex gap-1.5">
                {MANA_COLORS.map((c) => (
                  <button
                    ...
                    title={c.label}
                    className={cn(
                      "h-7 w-7 rounded-full text-[11px] font-bold ring-2 transition-all",
                      c.bg, c.text,
              ...
              <div className="flex gap-1">
                {RARITIES.map((r) => (
                  <button
                    ...
                    className={cn(
                      "px-2.5 py-1 rounded text-[10px] uppercase font-medium border transition-all",
        ...
        <div className="rounded-xl border border-border/60 bg-card overflow-hidden divide-y divide-border/40">
          {filtered.map((c) => (
            <div key={c.id} className="flex items-center gap-3 px-4 py-2.5 hover:bg-secondary/30 transition-colors">
              <button type="button" onClick={() => setOpenId(c.scryfall_id)} className="shrink-0 focus:outline-none">
                {c.image_url ? (
              ...
              <button type="button" onClick={() => setOpenId(c.scryfall_id)} className="flex-1 min-w-0 text-left">
              ...
              <div className="flex items-center gap-3 shrink-0 text-xs">
                {c.rarity && (
                  <Badge variant="outline" className={`text-[9px] uppercase hidden sm:inline-flex ${RARITY_CLASS[c.rarity] ?? ""}`}>{c.rarity}</Badge>
                )}
